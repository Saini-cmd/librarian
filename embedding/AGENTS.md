...
- **Progress**: `embed_texts` / `embed_chunks` accept an optional `progress(done_batches, total_batches)` callback (default `None`, no behavior change) — called after each batch completes, used by the eval CLI to render its embed progress bar
