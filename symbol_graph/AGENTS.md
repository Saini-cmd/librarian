...
- Not cached in-process anymore — graphs are persisted to Postgres (`repo_graphs` via `core/repositories/graph.py`) by the orchestrator at ingest time; `GET /graph` reads the DB (lazy fallback builds + persists for pre-existing repos, and rebuilds stored graphs whose `version` is below `GRAPH_VERSION`)
...
- Keep the builder pure — no DB/auth logic here; endpoints live in `backend/routers/repositories.py`, persistence in `core/repositories/graph.py`
