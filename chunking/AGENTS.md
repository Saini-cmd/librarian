...
## Ownership
- `chunk_model.py` — `CodeChunk` dataclass definition
...
- `chunk_pipeline.py` — Orchestrator: routes files to correct chunker, returns chunks directly; expects `file_metadata["repo"]` pre-populated by ingestion
...
- `CodeChunk` payload preserved through embedding → retrieval → RAG pipeline
