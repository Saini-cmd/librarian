...
- Conversation selection: on ingest, `AppPage` sets `activeConvId` from `/api/process`'s `conversation_id`; opening an already-indexed repo creates a fresh conversation via `createConversation` (new entry each open); clicking a sidebar entry opens its history (messages + citations) via `GET /api/conversations/{id}` — history loads show a **skeleton** (`messagesLoading` → `ChatMessages` `loading` prop), and `ChatMessages` is keyed by `activeConvId` so it remounts and pins to the bottom
...
- Header: left side shows the **active repo name as the heading** (truncates); right side holds a **Sync button + "Update available" badge** (driven by `getRepoUpdates`) and a **segmented control** (Chat ⇄ Graph pills, iOS style — active pill = `bg-primary text-primary-content`, labels hidden on mobile) . Graph view fetches `getRepoGraph(selectedRepoHash)` lazily (cached per hash in state) and renders `SymbolGraphView`; chat messages are preserved when switching views
...
- **Usage-cap 429 handling**: process/sync/chat-stream fetch failures (`!res.ok`) parse the error body via `readError` (`src/api/sse.js`), which returns `{message, quota}`. A quota 429 is stored in `quota` state and rendered as a `QuotaNotice` banner (start view above RepoInput, ready view below the header) — NOT thrown as a generic error. A chat-stream 429 removes **both** the optimistically-added user bubble and the empty placeholder (the message was never accepted/persisted — the backend rejects before `add_message`) and **restores the typed text into the composer** (`setDraft(userMsg)`) so nothing is lost; non-quota errors keep the generic message paths. `quota` clears on new action / dismiss
