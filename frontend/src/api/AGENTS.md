...
- Repo-scoped helpers: `getRepoGraph(repoName)` and `getFileSummary(repoName, filePath)` hit `/repositories/{repo}/graph` and `/repositories/{repo}/summary`; `getChunk(repoName, chunkId)` hits `/repositories/{repo}/chunks/{chunkId}` (full chunk incl. `content`) for clickable chat citations
