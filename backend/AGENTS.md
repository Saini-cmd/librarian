...
- Endpoints:
  ...
  - `GET /api/repositories/{repo_hash}/graph` — symbol graph for that commit (auth, ownership via `repo_url`; lazy build fallback, and lazy rebuild when the stored graph's `version` is stale vs `GRAPH_VERSION`)
