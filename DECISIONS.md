...
### D39 — Shared `core/` package (layering inversion)
- **Date**: 2026-08-21
- **Decision**: Extracted the shared foundation into a new top-level `core/` package that both the API layer and the domain packages depend on, inverting the previous layering where domain packages (`orchestration/`, `summarization/`, `memory/`, `tests/`) reached into `backend.state`/`backend.database`/`backend.models` for DB session, ORM, and data access. Moves: `backend/database.py` → `core/db.py`, `backend/models.py` → `core/models.py`, `backend/usage.py` → `core/usage.py`, root `prompts.py` → `core/prompts.py`, `normalize_repo_url` → `core/url.py`, and the 489-line `backend/state.py` was split into `core/repositories/{users,indexed_repo,conversations,graph}.py` + `core/url.py`. The Qdrant/DB admin wipe (`reset_index_state`, `collection_exists`, `COLLECTION_NAME`) moved to `backend/reset.py` — deliberately NOT in `core/`, since it touches Qdrant + the memory domain and `core/` must not depend on domains. `uvicorn backend.main:app`, `docker`, `dev.sh`, and test invocation are unchanged. ~29 import sites updated; verified via a 45-module import sweep + the usage/orchestrator/endpoint smokes.
- **Rationale**: `backend/` is the FastAPI shell but was also the shared data layer, so every pipeline feature pulled in the web package and `state.py` grew into an unmaintainable catch-all. A root `core/` gives a clean dependency rule (api + domains → core; core → nothing), a home for future shared primitives (config, LLM clients), and keeps the rest of the existing modular layout intact. `reset.py` stays in the API layer because admin wipe is an app-level action and keeping it out of `core/` preserves the "core never imports domains" invariant.
- **Alternatives rejected**: a full `app/` or `apps/`+`packages/` monorepo restructure (large churn, no payoff at this size — every AGENTS.md and import would move); keeping the inverted layering and only adding new code cleanly (leaves `state.py` and the existing violations); moving `reset` into `core/` (would make `core/` import `memory`/`vector_store` — breaks the invariant).
- **Note**: Historical decision entries (e.g. D32 "root `prompts.py`", D37 "`backend/usage.py`") describe paths as of their decision date; the current locations are `core/prompts.py` / `core/usage.py`.
